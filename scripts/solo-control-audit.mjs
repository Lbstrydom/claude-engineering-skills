...
async function cmdRun() {
  ...
  // Manual `--commits` gives bare real-commit shas (unchanged legacy shape,
  // `git show` is correct for them — see `extractDiff`'s docstring). Auto-
  // discovery gives `{commitSha, auditedSha, auditedTree}` records needing the
  // dirty/clean-branching `extractAuditedDiff`. Normalize both into one `units`
  // shape so the rest of this function doesn't branch per-item.
  ...
  if (units.length === 0) { log('No target commits found (no B/C audit-code shadow units yet).'); process.exit(0); }
  ...
  // re-audits everything. Resume key is `unitKey` (falls back to the legacy `.sha`
  // field for perCommit entries written before this composite key existed, where
  // `sha` alone WAS the resume key).
  ...
  const commits = units.filter((u) => !covered.has(u.resumeKey));
  ...
  for (const unit of commits) {
    ...
    out.perCommit.push({ sha, unitKey: unit.resumeKey, repo: path.basename(root), state: 'ran', findings: commitFindings, chunks: chunks.length, skippedSensitive: ext.skippedSensitive, conformanceByPass: commitConformance });
  }
...
export const _internals = { chunkDiff, continuationMarker, extractDiff, extractAuditedDiff, locateCommit, treeExists, partitionDiscoveredRows };

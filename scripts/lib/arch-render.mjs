 ...
 * the byte-determinism test depends on).
 *
 * ## The digest width is load-bearing, and 8 hex was not enough
 *
 * This carried an 8-hex (32-bit) prefix while describing itself as producing a
 * collision-free id. It does not: 32 bits has a ~50% birthday collision at
 * ~77k items, and at this repo's ~4.3k indexed symbols the probability of at
 * least one collision is already **~0.2%** — small, but the failure it causes is
 * the exact one this function exists to prevent (Mermaid silently merges the
 * nodes and the diagram drops a symbol with no error anywhere). A 1-in-500
 * chance of silently rendering a wrong graph is not a guarantee.
 *
 * Widened to 16 hex (64 bits), where the same birthday bound sits above 5×10⁹
 * items — comfortably beyond any repo this renders. A render-scoped uniqueness
 * table would make it *truly* collision-free, but that trades a pure function
 * for stateful bookkeeping in a diagram generator; widening the digest is the
 * smallest change that makes the claim true.
 *
 * **The guarantee is now stated exactly**: unique unless two distinct
 * `uniqueKey`s share a 64-bit sha256 prefix. Not "collision-free" — that was
 * the part that was false.
 *
 * Note this changes every generated anchor once. `docs/architecture-map.md` is
 * a Category-A artifact (gitignored, regenerated by `npm run arch:render`), so
 * the churn is not a committed diff.
 *
...
export function mermaidId(prefix, key, uniqueKey = key) {
  ...
  const digest = createHash('sha256').update(String(uniqueKey)).digest('hex').slice(0, 16);

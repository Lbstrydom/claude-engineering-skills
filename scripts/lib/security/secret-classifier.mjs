 ...
 *
 * Back-ported from the corporate `/security-strategy` hardening kit
 * (docs/plans/security/PLAN.md §4.4), adapted to this repo's gentle,
 * pattern-based redactor (`lib/secret-patterns.mjs`) rather than the kit's
 * `sanitizer.mjs` — this repo's `sanitizer.redactSecrets` blanket-redacts any
 * 20+ char token and would corrupt legitimate incident prose (e.g. long
 * identifier names). `secret-patterns.mjs` only touches genuine secret shapes.
 *
 * Two-tier policy:
 ...
 * low-confidence PII. Proper-names are detection-only: `classifySecrets`
 * reports them (so a caller that wants a human-review signal can read
 * `lowConfidence`), but `preWriteSecretGate` does NOT act on or emit an event
 * for them — a name match alone leaves the content `clean`.
 *
...
export function preWriteSecretGate(content) {
  ...
  // Defence in depth: run the shared pattern-based sanitizer for any
  // shape-specific leftovers. `.text` — this repo's redactSecrets returns
  // {text, redacted} (lib/secret-patterns.mjs).
  redacted = redactSecrets(redacted).text;

...
| the synced consumer bundle | **authoritative**: the synced `sync-isolation-verify` run *in the consumer*. `npm run sync:dry` from here is the pre-check, not the verdict | zero unexpected diffs; no orphans |

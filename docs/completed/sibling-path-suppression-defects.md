...
- **Date**: 2026-07-17
- **Status**: Complete — implemented + audited via `/cycle --autonomous`
  (2026-07-18). Plan audit: 4 GPT + 1 Gemini → APPROVE. Code: Cluster A 2
  rounds, Cluster B 1 round, consolidated Gemini gate → **APPROVE** (0 new, 0
  wrongly dismissed).
...
### 2026-07-18 — `/cycle --autonomous` (clustered)

**Completed**: all 5 phases. **Cluster A** — `buildBanditArmRows` (bucket can
never be null), reader+writer aligned on `GLOBAL_CONTEXT_BUCKET`, migration
`20260718090000` applied and verified live. **Cluster B** —
`shouldSuppressWithReason` (boolean `shouldSuppress` delegates; the pre-existing
suite passing untouched is the proof), `runLocalFpPass` + `runSuppressionPasses`,
the local loop lifted out of the ledger branch, `populateFindingMetadata`
hoisted above it, and a mutation-proven source-assertion call-site pin.

**Preflight caught three §7↔§7b drifts** before any execution (the same class the
plan audit kept finding): the `<ts>` placeholder, a `(modify)` on a file that
does not exist, and the call-site test in no phase's scope.

**Deviations from the plan, each earned from a finding**:
- **Migration gained a `LOCK TABLE`** (R2-M2) — a genuine check-then-act race:
  the preflights READ and the ALTERs ACT, so a stale consumer could insert an
  invalid row in between and the ALTER would fail with a raw error instead of
  the guided RAISE. Distinct from the lock-*timeout* ceremony §7 rejected: `SET
  NOT NULL` takes `ACCESS EXCLUSIVE` anyway; taking it up front is what makes
  the preflight mean anything.
- **Preflight + recovery cover `''`, not just NULL** (R1-M4/M5) — the migration
  recognised empty as invalid in its `CHECK` but never preflighted it, so a
  drifted consumer would pass every gate and then fail on `ADD CONSTRAINT` with
  no guidance: worse than the failure the preflight exists to prevent.
- **Both duplication findings unified, not pragma-suppressed** (R1-M16/M17) —
  `partitionByVerdict` + `runFpPass` are now single implementations; the two
  named passes are thin adapters. Mirroring the cloud shape was the intent;
  copying its body was not.

**The sharpest moment — and the one worth remembering.** The refactor left a
dangling `cloudPass` reference at `:2777`: a `ReferenceError` crashing the
orchestrator on **every cloud-enabled R2+ run**. 180 scoped tests and **6,767
suite tests were all green**, because nothing executes `runLegacyProductionAudit`.
Only running the real audit caught it. That produced a dangling-reference pin, a
module-load test, and a spawned plan for the underlying gap (M15 — the audit's
own diagnosis: *"critical composition behavior is protected by source grepping"*).

**Four independent HIGHs spawned rather than buried** — most notably
`upsertPromptVariant`'s conflict key omitting `repo_id`: **the fifth instance of
this plan's own defect class**, live across the three repos on this DSN, and now
part of the §Out-of-Scope lint's acceptance set.

**Close-out**: migration idempotent (`applied 0, skipped 68`), no drift,
`bandit_arms` still **20 rows / 100% `'global'`** (growth would be the 403k
tell). Scope suites 180/180.

> **Honest caveat on the fix-gate.** The full suite has 5 failures, **all
> attributable to a concurrent session** mid-change on `schemas.mjs` (a
> `PROVIDER_FACING_SCHEMAS` registry) and `tiered-pipeline.mjs` (stage0
> evidence-triage). Verified, not assumed: the failing pins assert on
> `tiered-pipeline.mjs:422` and `schemas.mjs`, neither of which this change
> touches, and this diff contains zero schema code. So the honest claim is *"my
> scope is green and the remainder is attributable elsewhere"* — **not** a green
> full suite.
...
> **Corrected at `/cycle`'s fail-closed preflight (2026-07-17)** — three
> §7-vs-§7b drifts, the same class the audit kept finding, now in the block
> `/cycle` actually executes: (1) Phase 2 still carried the `<ts>` placeholder
> that R1-H1 replaced in §7 — an unresolvable derived scope; (2) Phase 3 named
> `tests/findings-tracker.test.mjs`, which **does not exist** (a `(modify)` must
> resolve on disk — `shouldSuppress` is pinned in `tests/shared.test.mjs`), with
> a malformed tag carrying a "locate at implementation" deferral; (3)
> `tests/suppression-call-site.test.mjs` — added to §7 by R3-M2 — appeared in
> **no** phase, so Cluster B's audit would have excluded the very test pinning
> its central invariant. Each correction takes the value §7 or the filesystem
> already established; no design changed.
...
+ non-empty `CHECK` migration. Files:
`supabase/migrations/20260718090000_bandit_arms_bucket_not_null.sql` (create).
...
add the call-site pin. Files:
`scripts/lib/audit/legacy-production-audit.mjs` (modify),
`tests/suppression-call-site.test.mjs` (create).

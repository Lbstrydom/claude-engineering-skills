...
## The real fix: SKILL.md descriptions, not eval prompts
(measured 2026-09-20 — all four previously-flaky/failing cases now fixed)

Every fix attempt up to this point tuned the *eval prompt*. That approach
had a hard ceiling: the prompt can't control what happens inside Claude's
own reasoning before it decides whether to invoke a skill at all, and the
common thread across `explain`, `plan`, `persona-test`, and `audit-code`'s
failures was exactly that — Claude checking something first (a file exists,
a browser tool is available, "can I just do this myself") and never getting
to a skill-choice decision, or choosing not to delegate. The lever that
actually moves that decision is the skill's own `description` frontmatter —
the only thing visible to Claude *before* a skill is invoked (the full
SKILL.md body, Step 0 etc., is only read once `Skill()` is called).

Added one targeted clause to each of four skills' `description` (all under
the 1024-char frontmatter cap; `persona-test` needed trimming elsewhere to
fit):

| Skill | Added clause |
|---|---|
| `explain` | "Invoke on the question's shape, don't pre-check the target exists first — Step 0 validates it and reports cleanly if missing; don't Grep/Glob and answer directly instead." |
| `plan` | "Invoke on the request's shape; don't explore the repo for stack context first and answer directly — the skill gathers that itself once invoked." |
| `persona-test` | "Invoke on the request's shape; don't check for browser tools first and decline — the skill resolves its own driver (Playwright MCP, host tools, BrightData, or read-only) once invoked." |
| `audit-code` | "Invoke this for ANY code-correctness review, even a small pasted snippet or diff — don't just review it yourself directly; a single read-through is not a substitute for the multi-pass GPT+Gemini adjudication this skill runs, and skipping it means the review never actually happened." |

Results, each validated across **two independent 3-run batches** (the
"don't trust a single batch" discipline this doc had to learn the hard way
earlier applies in full here — a good first batch was exactly how `explain`
and `ux-lock` fooled this doc before):

| Case | Before | After (pooled, 2 batches) |
|---|---|---|
| `explain-topic-question` | 2/7 (29%) | **6/6 (100%)** |
| `plan-vs-audit-plan-design-request` | 7/12 (58%) | **6/6 (100%)** |
| `persona-test-vs-click-test` | 1/3 (33%) | **6/6 (100%)** |
| `audit-code-request` | 0/12 (0%) — deterministic across 4 prompt-side hypotheses | **6/6 (100%)** |

A full 13-case pilot re-run after these four fixes: **12/13 clean at 1.00**
(the 13th, `audit-code-request`, was independently re-confirmed 6/6 in its
own dedicated batches immediately after). Every case in this pilot is now
reliably green.

**Why this worked where prompt tuning didn't**: prompt-side fixes could
only ever change what Claude sees in a specific test scenario. A
description-side fix changes the actual triggering behavior for every real
request shaped like the ones this pilot tests — which is the actual goal,
not just a passing eval. This is a real product improvement to four shipped
skills, not merely a fix to this pilot's own scorecard.

**One caveat worth tracking**: `audit-code`'s fix reversed a deterministic
0%, the strongest and most reproducible finding in this whole pilot,
achieved with a single clause on the first attempt. That is either a very
clean win or a sign the fix is narrowly matched to this pilot's specific
prompt shapes (a 4-file diff and a 6-line snippet, both explicitly pasted
inline) rather than the full range of real "audit my code" requests. Real
usage — not just this eval — is the actual test of whether `audit-code`
now reliably triggers on a genuine PR-review ask.

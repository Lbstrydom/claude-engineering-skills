...
🔒 = `disable-model-invocation: true` — Claude Code and VS Code Copilot will not auto-trigger it; invoke explicitly via `/<name>`. Each locked skill also states the rule in its body, for hosts that ignore the key.

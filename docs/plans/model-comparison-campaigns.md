...
| ~~`scripts/build-dashboard.mjs`~~ → `render.mjs` + `collect-reference.mjs` + `schema.mjs` | modify | **CORRECTED at implementation.** The four integration points live one layer below the CLI wrapper: nav entry + renderer call in `render.mjs` (REGISTRY + SLICERS), collector invocation + degraded-status propagation in `collect-reference.mjs`, and payload survival in `schema.mjs` — Zod strips undeclared keys and `renderDocument` builds sections from the PARSED object, so an undeclared payload is deleted rather than passed through. `build-dashboard.mjs` itself was not modified. |

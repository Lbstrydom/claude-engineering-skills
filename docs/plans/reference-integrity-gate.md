...
| R19 | **R3-M3 — new CLIs must join the repo's CLI lifecycle contract.** | `scripts/check-docs-refs.mjs` and `scripts/check-plan-status.mjs` each implement the `--selfcheck-relocation` handler and join `CLI_SMOKE_SET` + `scripts/.cli-catalog.json` (AGENTS.md "CLI smoke contract"). `scripts/lib/plan-status.mjs` is a library (no `main()`) → it gets an import-test in `tests/relocation-guard.test.mjs` instead, per the same contract. Folded into Phases 1 and 4 rather than close-out, because close-out commands cannot prove registration. |

...
- **Date**: 2026-09-20
- **Status**: Complete — verdict: keep Arm A (incumbent). See
  [`docs/research/experiment-5-reviewer-cost-value.md`](../research/experiment-5-reviewer-cost-value.md).
- **Author**: Claude + Louis
- **Scope**: backend
...
| `docs/research/experiment-5-reviewer-cost-value.md` | create | Verdict document: corpus, manifest (resolved ids, transports, pricing version), run matrix with cell states, per-cohort per-arm table, `decide` output verbatim, what the margin does and does not claim. Written **after** `score`, never before. **Done 2026-09-23**: winner A, second-rater-validated. | — |
...
**Done 2026-09-23 — with two deviations from this phase's own plan, both
stated in the verdict doc**: (1) the `sev`/`sevReason` columns and the
adjudication rubric's 40-row cap were never wired into the actual labeling
session (the rubric file existed from Phase 1 but the labeling prompt never
referenced it) — scoring used emitted severity throughout, not adjudicated
severity; (2) a single human-labeled sheet was escalated to a full,
independent cross-vendor second pass (GPT judging all 1,551 rows) after a
10% sample showed the human rater's `false` calls were unreliable (one
cluster independently confirmed wrong by hand). `winner: A` (incumbent) —
confirmed identically by both judges' `decide()` output despite only 64.9%
row-level agreement between them; the default does not change and the
`audit-code` broad-trigger clause is not reverted.
...
- **2026-09-22 merge-clustering instrument fix (post-Phase-3, pre-Phase-4)**: a real merge run went silent for 2.5 wall-clock hours and was killed as presumed-hung before it was understood that the clustering step's `cli`-backend subprocess spin-up (one `claude -p` call per <=50-row commit batch, dozens of sequential calls, zero progress output) was very likely still legitimately working — the per-call arithmetic matched the observed wall-clock almost exactly. Fixed with `backend:'sdk'` (removes the subprocess overhead) plus a per-commit progress log line (removes the silence), both locked with a source-text regression test since `cmdMerge` is a CLI-only subcommand. Committed `e114ff0f`.
- **2026-09-23 Phase 4 complete — verdict: keep Arm A**. Full detail, both judges' `decide()` output, and the stated deviations from this plan (adjudicated `sev`/`sevReason` never wired into the labeling session despite the Phase-1 rubric existing; a single-rater sheet escalated to a full independent second pass after a 10% sample showed the rater's `false` calls were unreliable) are in [`docs/research/experiment-5-reviewer-cost-value.md`](../research/experiment-5-reviewer-cost-value.md). Headline: both a Claude-family rater and an independent GPT rater agree exactly on `eligible`/`ineligible`/`winner` (A wins; both cold arms C and E are ineligible on `false-rate>0.33`) despite only 64.9% row-level label agreement between them — the row-level noise is largely non-differential. Total spend: $194.76 of $350 (arm-cost ledger) + ~$2–3 separate GPT-judge verification cost.

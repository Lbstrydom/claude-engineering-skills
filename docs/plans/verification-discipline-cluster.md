...
|---|---|---|
| **R1** | New SKILL.md prose trips `check-gate-contracts.mjs`'s candidate scanner and fails `skills:check` at push. | Known before writing (Code Trace); Phase 2 lands each step **with** its contract entry. Designed for, not discovered. |

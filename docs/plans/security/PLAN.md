...
11. `cross-skill.mjs get-incident-neighbourhood --json '{"repo":"X","targetPaths":["a","b"],"k":5}'`: returns at most 5 rows sorted by path_overlap DESC, similarity DESC NULLS LAST. *(Corrected 2026-07-20: this criterion originally showed a `--paths a,b` csv form. `arch-memory-planning-anchor.md` R3-M1 later fixed every cross-skill sub-command on a single `--json` payload interface — "No `--paths` csv form" — and the handler reads `targetPaths` from the payload. The csv form never existed; a caller following the old text had the flag silently ignored.)*

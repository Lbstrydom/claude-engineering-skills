...
15 checks, run as independent subprocesses (never crashes the whole run when
...
| `workflow-cadence` | *(none — asks GitHub about THIS repo's crons, [docs/reference/workflow-cadence-doctor.md](../reference/workflow-cadence-doctor.md))* | `node scripts/workflow-cadence-doctor.mjs --strict` — did each workflow in `.workflow-cadence.json` actually run, and pass, on its declared triggering event? A cron that stops firing produces no run, no failure and no notification, so nothing else here can see it. `--strict` is required because `runCheck` keys status on the exit code and the doctor is advisory by default. Watches with `requireMeasurement` also read each success's check-run annotations for the `audit-loop-no-measurement` marker (a green run that skipped its work is `unmeasured`, not `ok`). | `GITHUB_TOKEN` (`actions: read`; `checks: read` for `requireMeasurement`) |

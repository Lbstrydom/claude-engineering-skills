...
Five properties worth knowing, each verified by running the branch:
...
- It **converges, not just overlays**: after copying, it deletes anything under
  `scripts/.claude-skills/` that no longer exists in the main checkout's copy —
  a file deleted or renamed upstream does not survive in an already-hydrated
  worktree forever (fixed 2026-09; `fs.cpSync` alone only adds/updates).

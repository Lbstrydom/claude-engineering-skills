...
describe('COVERAGE — no z.toJSONSchema call site escapes the registry', () => {
  it('every provider-facing schema handed to z.toJSONSchema is registered', () => {
    ...
    const ALLOW = new Map([
      ...
      ['VisualContractSchema', 'check-visual-contract-doc-drift.mjs NOT provider-facing — aged-out-acceptance-remainder.md §4 (e3da8d42). z.toJSONSchema(VisualContractSchema) is read ONLY to diff its `properties` key set against the visual-audit contract reference doc\'s own JSONC example; the JSON form is never sent to any provider, and VisualContractSchema\'s two `.refine()` calls (theme-name/surface-id uniqueness) are deliberately allowed to drop from that comparison since the drift gate compares KEY SETS, not runtime validation. The two matches (one real call site, one in this file\'s own docstring naming it) both key on this same name.'],
    ]);

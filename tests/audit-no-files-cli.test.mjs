 ...
 *
 * No API call is made: the guard throws BEFORE any GPT pass, so a dummy key suffices.
 */
...
test('CLI: an explicit but unreadable --diff (the base..HEAD range-misuse) fails fast', () => {
  ...
  try {
    ...
    execFileSync(process.execPath, ['scripts/openai-audit.mjs', 'code', plan, '--scope', 'plan', '--diff', 'HEAD~1..HEAD', '--out', out], {
      encoding: 'utf8',
      env: { ...process.env, OPENAI_API_KEY: 'sk-test-dummy-key', GEMINI_API_KEY: '', ANTHROPIC_API_KEY: '', MODEL_CATALOG_REFRESH: 'skip', LEARNING_DISABLE: '1', AUDIT_DB_URL: '' },

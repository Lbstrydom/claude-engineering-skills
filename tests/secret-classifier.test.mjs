 ...
 * (scripts/lib/security/secret-classifier.mjs). Back-port: docs/plans/security.
 */
...
test('legitimate long identifiers in prose are NOT mangled', () => {
  // Regression: this repo's sanitizer.redactSecrets blanket-redacts any 20+
  // char token; the gate deliberately uses the gentle pattern-based redactor
  // so incident prose mentioning long symbol names survives intact.
  const r = preWriteSecretGate('the getSecurityIncidentsByRepo accessor reads the index');
  assert.equal(r.kind, 'clean');
  assert.match(r.content, /getSecurityIncidentsByRepo/);
});
